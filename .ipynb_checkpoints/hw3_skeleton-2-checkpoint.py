...
def parse_data(user_action_rdd, product_category_rdd):
    ...
    print(user_action_parsed, product_category_parsed)
...
def analyze_user_behavior(filtered_data):
    ...
    # (user_name, max_count)
    user_max_count = user_product_counts.map(lambda x: (x[0][0], (x[0][1], x[1]))).groupByKey().mapValues(lambda x: max(x[1]))
    # (user_name, (product_name, n))
...
# Call main function
main(sc, 'user_actions.csv', 'product_categories.csv')
